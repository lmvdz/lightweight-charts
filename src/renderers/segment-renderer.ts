...
export class SegmentRenderer implements IPaneRenderer {
	...
	public hitTest(point: Point, renderParams: CanvasRenderParams): HitTestResult<void> | null {
		...
		const line = this._extendAndClipLineSegment(this._data.points[0], this._data.points[1], renderParams);
		if (null !== line && distanceToSegment(line[0], line[1], point).distance <= tolerance) { return this._hitTest; }
		return null;
	}

	private _extendAndClipLineSegment(end0: Point, end1: Point, renderParams: CanvasRenderParams): Point[] | null {
		const data = ensureNotNull(this._data);
		return extendAndClipLineSegment(end0, end1, renderParams.cssWidth, renderParams.cssHeight, !!data.line.extend?.left, !!data.line.extend?.right);
	}
