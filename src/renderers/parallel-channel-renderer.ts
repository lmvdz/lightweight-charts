...
export class ParallelChannelRenderer extends ScaledRenderer {
	...
	public hitTest(point: Point, renderParams: CanvasRenderParams): HitTestResult<void> | null {
		...
		const firsLineHitResult = this._extendAndHitTestLineSegment(point, end0, end1, renderParams);
		if (firsLineHitResult !== null) { return firsLineHitResult; }
		if (this._data.points.length === 4) {
			const [,, end2, end3] = this._data.points;
			const secondLineHitResult = this._extendAndHitTestLineSegment(point, end2, end3, renderParams);
			...
			if (this._data.showMiddleLine) {
				...
				const middleLineHitResult = this._extendAndHitTestLineSegment(point, end4, end5, renderParams);
	...
	protected _drawImpl(ctx: CanvasRenderingContext2D, renderParams: CanvasRenderParams): void {
		...
		const [end0, end1] = this._data.points;
		this._extendAndDrawLineSegment(ctx, end0, end1, renderParams);

		if (this._data.points.length === 4) {
			const [,, end2, end3] = this._data.points;
			this._extendAndDrawLineSegment(ctx, end2, end3, renderParams);
			this._drawBackground(ctx, this._data.points, renderParams);
			...
			if (this._data.showMiddleLine) {
				...
				const end4 = end0.add(end2).scaled(0.5);
				const end5 = end1.add(end3).scaled(0.5);
				this._extendAndDrawLineSegment(ctx, end4, end5, renderParams);
			}
		}
	}

	protected _extendAndDrawLineSegment(ctx: CanvasRenderingContext2D, end0: Point, end1: Point, renderParams: CanvasRenderParams): void {
	...
	protected _extendAndClipLineSegment(end0: Point, end1: Point, renderParams: CanvasRenderParams): Segment | null {
		...
		return extendAndClipLineSegment(end0, end1, renderParams.cssWidth, renderParams.cssHeight, !!data.extend?.left, !!data.extend?.right);
	}

	protected _drawBackground(ctx: CanvasRenderingContext2D, points: Point[], renderParams: CanvasRenderParams): void {
		const data = ensureNotNull(this._data);
		const [end0, end1, end2, end3] = points;

		if (equalPoints(end0, end1) || equalPoints(end2, end3)) { return; }
		if (renderParams.cssWidth <= 0 || renderParams.cssHeight <= 0) { return; }
		if (distanceToLine(end0, end1, end2).distance < 1e-6 || distanceToLine(end0, end1, end3).distance < 1e-6) { return; }

		const { cssWidth: width, cssHeight: height } = renderParams;

		let computedPoints: Point[] | null = [new Point(0, 0), new Point(width, 0), new Point(width, height), new Point(0, height)];
